    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card shadow-card">
        <div className="container max-w-3xl mx-auto flex items-center gap-4 py-4 px-6">
          ...
          <div>
            <h1 className="text-lg font-heading font-semibold text-foreground">{t("transactions.title")}</h1>
            <p className="text-xs text-muted-foreground">{t("transactions.subtitle")}</p>
          </div>
        </div>
      </header>
      ...
      <main className="container max-w-3xl mx-auto px-6 py-10">
        {transactions.length === 0 ? (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
            ...
            <h2 className="text-xl font-heading font-semibold text-foreground mb-2">{t("transactions.noTransactions")}</h2>
            <p className="text-muted-foreground mb-6 text-sm">{t("transactions.noTransactionsDesc")}</p>
            <Button onClick={() => navigate("/dashboard")}>{t("payment.backToDashboard")}</Button>
          </motion.div>
        ) : (
          <div className="space-y-3">
            {transactions.map((txn, index) => (
              <motion.div key={txn.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                className="bg-card rounded-xl border p-5 shadow-card flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${txn.type === "refund" ? "bg-accent/20" : "bg-primary/10"}`}>
                    {txn.type === "refund" ? <ArrowDownLeft className="w-5 h-5 text-accent-foreground" /> : <ArrowUpRight className="w-5 h-5 text-primary" />}
                  </div>
                  <div>
                    <p className="font-semibold text-foreground font-body text-sm">
                      {txn.description || (txn.type === "payment" ? t("transactions.monthlyPayment") : t("transactions.refund"))}
                    </p>
                ...
                <div className="text-right">
                  ...
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize ${getStatusColor(txn.status)}`}>{txn.status}</span>
                </div>
              </motion.div>
            ))}
          </div>
