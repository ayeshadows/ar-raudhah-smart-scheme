...
type Application = {
  ...
  full_name: string; nric: string; cancelled_at: string | null;
};
...
const StatusPage = () => {
  ...
  const handleCancelApplication = async () => {
    ...
    else { toast.success("Application cancelled"); await fetchApplications(userId); }
    setCancelling(false); setCancelDialogOpen(false); setCancellingAppId(null);
  };
  ...
  const getSteps = (status: string) => {
    switch (status) {
      case "approved": return [
        { label: t("status.submitted"), done: true }, { label: t("status.underReview"), done: true },
        { label: t("status.paymentVerified"), done: true }, { label: t("status.approved"), done: true },
      ];
      case "rejected": return [
        { label: t("status.submitted"), done: true }, { label: t("status.underReview"), done: true },
        { label: t("status.rejected"), done: true },
      ];
      case "cancelled": return [
        { label: t("status.submitted"), done: true }, { label: t("status.cancelled"), done: true },
      ];
      default: return [
        { label: t("status.submitted"), done: true }, { label: t("status.underReview"), done: false },
        { label: t("status.paymentVerified"), done: false }, { label: t("status.approved"), done: false },
      ];
    }
  };
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card shadow-card">
        <div className="container max-w-3xl mx-auto flex items-center gap-4 py-4 px-6">
          ...
          <div>
            <h1 className="text-lg font-heading font-semibold text-foreground">{t("status.title")}</h1>
      ...
      <main className="container max-w-3xl mx-auto px-6 py-10">
        ...
        {applications.length === 0 ? (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
            ...
            <h2 className="text-xl font-heading font-semibold text-foreground mb-2">{t("status.noApplications")}</h2>
            <p className="text-muted-foreground mb-6 text-sm">{t("status.noApplicationsDesc")}</p>
          ...
          <div className="space-y-6">
            {applications.map((app, index) => {
              ...
              return (
                <motion.div key={app.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      {app.cancelled_at && (
                        <p className="text-xs text-muted-foreground mt-1">
                          {t("status.cancelledOn")} {new Date(app.cancelled_at).toLocaleDateString()}
                        </p>
                      )}
                    </div>
                    ...
                    {steps.map((s, i) => (
                      <div key={i} className="flex items-center flex-1">
                        <div className="flex flex-col items-center flex-shrink-0">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold ${s.done ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                            {s.done ? "✓" : i + 1}
                          </div>
                          <span className="text-[10px] text-muted-foreground mt-1.5 text-center max-w-[80px] leading-tight">{s.label}</span>
                        </div>
                        {i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-1 mt-[-16px] ${s.done ? "bg-primary" : "bg-muted"}`} />}
                      </div>
                    ))}
                  </div>
                  {app.status === "pending" && (
                    <div className="mt-4 pt-4 border-t">
                      <Button variant="outline" size="sm" className="text-destructive border-destructive/30 hover:bg-destructive/10"
                        ...
                        <XCircle className="w-4 h-4 mr-1" /> {t("status.cancelApplication")}
                      </Button>
                    </div>
                  )}
                </motion.div>
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <DialogClose asChild><Button variant="outline">{t("dashboard.keepApplication")}</Button></DialogClose>
